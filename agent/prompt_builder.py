...
PLATFORM_HINTS = {
    ...
    "feishu": (
        ...
        "links are supported. "
        "For Feishu /docx/ document body work (read, preview, replace, update, or write), "
        "use only feishu_doc_* tools. Do not use feishu_drive_add_comment as a fallback for "
        "document-body changes, and do not offer a local Markdown file unless a Feishu API "
        "operation has actually failed and the user explicitly asks for a local file. "
        "For Feishu /base/ Base/Bitable URLs, use feishu_bitable_* tools, normally "
        "feishu_bitable_read_table for requests like '读取 Project Tracking', '读取每一个格子', "
        "'总结这个任务表', or '找出所有 overdue task'. Do not use read_file or browser scraping "
        "for Base data when the Bitable API is available. For Feishu Base/Bitable cell updates, "
        "always use feishu_bitable_prepare_record_update first, then apply only with "
        "feishu_bitable_apply_record_update after exact confirmation '确认修改 CHANGE_ID'. "
        "For meeting-driven Project Tracking plus Meeting Records plus Action Items writes, "
        "especially messages containing Project, Maison, and Meeting Notes, use "
        "the feishu-base-workflow skill and call feishu_bitable_prepare_project_meeting_write first. "
        "This is a deterministic workflow with a strict tool budget: one high-level prepare call for preview, "
        "then one high-level apply call after confirmation. Do not manually call feishu_bitable_read_table, "
        "feishu_bitable_list_records, feishu_bitable_list_fields, or feishu_bitable_list_views to prepare this write; "
        "the high-level tool does its own Base discovery and reads. "
        "If Maison, Project, Meeting Notes, and a Feishu Base URL or configured app_token are not all available, "
        "ask for the missing input and stop without calling Feishu tools. "
        "If feishu_bitable_prepare_project_meeting_write or feishu_bitable_apply_project_meeting_write returns an error, "
        "show the concise Chinese error and stop; do not try old meeting tools, single-cell update tools, alternate views, "
        "skills, comments, Docs tools, local files, browser automation, terminal commands, or manual workarounds in the same turn. "
        "Ask only '是否确认写入飞书？请回复：确认', "
        "and apply only with feishu_bitable_apply_project_meeting_write after a plain reply like '确认', "
        "'同意', '可以', '写入', or '确认写入'. Resolve the latest scoped pending preview internally; "
        "never show or ask the user to type a Change ID. Do not create fbu_* single-cell changes for this workflow. "
        "Create exactly one Meeting Records row and real Action Items records; never reuse blank rows, "
        "and verify idempotency plus DuplexLink fields before reporting success. "
        "Never substitute Feishu comments, Feishu Docs tools, local files, browser automation, "
        "or terminal editing for Base record updates. For /wiki/ URLs, inspect and resolve "
        "the underlying object before selecting docx or bitable tools. "
        "Single-cell writes still require the exact confirmation text '确认修改 CHANGE_ID'; direct-change requests "
        "must not bypass preview confirmation. "
        "You can send media files natively: include MEDIA:/absolute/path/to/file "
